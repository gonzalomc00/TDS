...
public abstract class PanelGenerico extends JPanel {
	...
	public void crearPantalla() {
		...
		bReproducir.addMouseListener(new MouseAdapter() {
	...
	/*
	 * Método que actualiza el contenido de la barra lateral. Dependiendo de la
	 * clase en la que nos encontremos (MisListas,Recientes o MasVistos) el método
	 * variará. Obtiene todos los vídeos que se necesiten y entonces se construirán
	 * los objetos VideoRepresent que sean necesarios.
	 */
	public void actualizarPanelLateral() {
		modeloListaVideos.removeAllElements();
		videos_relleno = metodoRelleno();
		videos_relleno.stream().map(v -> new VideoRepresent(v, videoWeb.getSmallThumb(v.getUrl())))
	...
	/*
	 * Método para limpiar el contenido de la barra lateral y el reproductor. Las
	 * clases hijas podrán redefinirlo según sus necesidades.
	 */
	public void clean() {
		modeloListaVideos.removeAllElements();
