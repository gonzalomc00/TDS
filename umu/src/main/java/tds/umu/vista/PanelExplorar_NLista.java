...
public class PanelExplorar_NLista extends JPanel {
	...
	// Ya que no queremos que en el modo Nueva Lista se reproduzcan vídeos cuando
	// hacemos doble click en los resultados de la búsqueda, añadiremos el
	// MouseListener únicamente cuando
	...
	public PanelExplorar_NLista(VentanaPrincipal ventana) {
		...
		// Inicializamos el MouseListener anteriormente descrito. Al hacer doble click
		// sobre un vídeo se llamará a la Ventana Principal para que actualice el
		// contenido del panel central de la
		// aplicación para que muestre el reproductor con el vídeo que hemos
		// seleccionado.
	...
	/*
	 * Metodo el cual llama a las distintas funciones para contruir el contenido de
	 * la ventana. .
	 */
	 ...
	 * Este panel solo estará visible en el modo Nueva Lista.
	 */
	private void construirPanelListaVideos() {
		...
		// MANEJADORES
		/*
		 * Al hacer click en este botón se buscará una lista de vídeo que pertenezca al
		 * usuario o se creará en su defecto.
		 */
		...
		/*
		 * Al hacer click en este botón se preguntará al usuario si quiere eliminar la
		 * lista que está actualmente seleccionada.
		 */
		bEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (lv_creada == null) {
					JOptionPane.showMessageDialog(null, "Debes tener una lista de videos seleccionada", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
					int input = JOptionPane.showConfirmDialog(null,
							"¿Quieres eliminar la lista de videos " + lv_creada.getNombre() + "?", "Borrar lista",
		...
		/*
		 * Al hacer click en este botón añadimos a la lista de video seleccionada el
		 * video seleccionado en la búsqueda.
		 */
		...
		/*
		 * Al hacer click en este botón eliminamos de la lista de video seleccionada el
		 * video seleccionado en la búsqueda.
		 */
	...
	/*
	 * Método para construir la vista del panel que contiene las etiquetas, el cual
	 * solo será visible en el modo Explorar.
	 */
	public void construirPanelEtiquetas() {
		...
		lista_etiquetas_sel.addListSelectionListener(new ListSelectionListener() {
		...
		/*
		 * Si seleccionamos una etiqueta de la primera lista de etiquetas y esta no se
		 * encuentra ya seleccionada, entonces la añadimos a la segunda lista de
		 * etiquetas, la cual filtrará la búsqueda.
		 */
		lista_etiquetas.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent event) {
		...
		/*
		 * Si seleccionamos una etiqueta de la segunda lista de etiquetas la eliminamos
		 * de ella.
		 */
	...
	/*
	 * Método para construir el panel que contendrá los componentes para realizar la
	 * búsqueda, visible en ambos modos.
	 */
	public void construirPanelBusqueda() {
		...
		// MANEJADORES
		/*
		 * Al hacer click en este botón eliminaremos todos los resultados de la
		 * búsqueda.
		 */
	...
	/*
	 * Métodos para actualizar el panel central de búsqueda con los resultados de la
	 * misma. Se construirá un objetos VideoRepresent por cada vídeo encontrado y
	 * entonces se añadirá a la lista para que puedan ser representados.
	 *
	 * La búsqueda estarña filtrada con las etiquetas seleccionadas en el panel
	 * derecho ( en el caso del modo Nueva Lista no habrá ninguna seleccionada por
	 * defecto, pues en este modo no queremos filtrar por etiquetas)
	 */
	public void actualizarListaVideos() {
		...
		videos_encontrados.stream().map(v -> new VideoRepresent(v, videoWeb.getSmallThumb(v.getUrl())))
	...
	/*
	 * Método utilizado para cargar las listas del panel derecho con todas las
	 * etiquetas encontradas en la base de datos. Como de las etiquetas solo nos
	 * interesa su nombre, hacemos un stream para pasar del objeto etiqueta a un
	 * String con su nombre.
	 */
	public void actualizarEtiquetasExplora() {
		List<Etiqueta> lista_etiquetas_cat = controlador.getEtiquetas();
		lista_etiquetas_cat.stream().map(etq -> etq.getNombre()).forEach(nom -> modeloEtiqDisponibles.addElement(nom));
	...
	/*
	 * Método utilizado para actualizar el contenido del panel izquierdo. Si el
	 * usuario ha introducido el nombre de una playlist y esta no existe, se
	 * procederá a preguntar si quiere crearla y la deja seleccionada por si quiere
	 * introducir vídeos en ella. En caso de existir, se seleccionará la lista y se
	 * actualizará el panel para mostrar todos los vídeos que contiene.
	 */
	private void actualizacionPlaylist() {
		if (!campoNombre.getText().equals("")) {
			lv_creada = controlador.obtenerLista(campoNombre.getText());
			// Si el nombre de la playList introducida no existe entonces se produce a
			// preguntar si se quiere crear,
			// y hacerlo si corresponde.
			if (lv_creada == null) {
				int res = JOptionPane.showConfirmDialog(this,
						"¿Quieres crear la lista de videos " + campoNombre.getText() + "?", "Crear lista de video",
						JOptionPane.YES_NO_OPTION);
	...
	/*
	 * Método utilizado para actualizar el panel izquierdo con todos los vídeos que
	 * contiene una lista de vídeo.
	 */
	private void actualizarVideosLista() {
		modeloVideosLista.removeAllElements();
		lv_creada.getVideos().stream().map(v -> new VideoRepresent(v, videoWeb.getSmallThumb(v.getUrl())))
	...
	/*
	 * En el modo Explorar ocultamos el panel izquierdo y añadimos el MouseListener
	 * que permite reproducir un vídeo si se hace doble click sobre él en la lista.
	 * Para evitar problemas por si se selecciona dos veces seguidas el panel
	 * Explorar, comprobamos que este no esté ya introducido.
	 */
	...
	/*
	 * En el modo Nueva Lista ocultamos el panel derecho y mostramos el izquierdo, a
	 * la vez que eliminamos el Mouse Listener que permite la visualización con
	 * doble click.
	 */
	...
	/*
	 * Método utilizado para limpiar todos los elementos del panel: la lista de
	 * etiquetas y etiquetas seleccionada, la búsqueda, la lista seleccionada...
	 * Este método será llamado siempre que accedamos al panel.
	 */
	public void renovar() {
